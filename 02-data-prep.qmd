...
# Build a binary missingness matrix (1 = missing, 0 = observed)
miss_mat <- df %>%
...
# t(M) %*% M produces the count of rows where BOTH columns are 1
intersection_matrix <- crossprod(miss_mat)
total_miss_counts <- diag(intersection_matrix)

# Conditional probability given var1 is missing, what % of the time
# is var2 also missing?
...
Sunshine missingness is station-conditional rather than weather-conditional. The same stations record missing sunshine on both dry and rainy days at nearly identical rates, because the instrument is absent from those stations entirely. This finding clears the imputation path: there is no outcome-related confound, and the missingness flags serve as optional station-level controls rather than necessary bias corrections.
...
The diagnostic evidence from @sec-missingness-diagnostics motivates each design decision in the imputation pipeline. Missingness is station-level rather than weather-conditional (@sec-weather-conditionality), temporally structured with extended contiguous gaps rather than random scatter (@sec-temporal-structure), and co-located across variables at the station level (@sec-co-missingness). These properties rule out simple listwise deletion, mean imputation, and weather-conditional correction. They call for a pipeline that respects temporal continuity for smoothly-evolving variables, exploits cross-variable correlations for the structured variables, and declines to impute where the data is genuinely unrecoverable.

### Stage 1: Missingness Flags

Before imputation, binary indicator variables are created for the four high-missingness targets. A flag value of 1 marks an observation subsequently imputed. Although @sec-weather-conditionality confirmed that sunshine missingness is not weather-conditional in aggregate, flagging still serves a purpose: it allows the fitted model to test whether the fact of imputation at a particular station is itself predictive of the outcome, absorbing any residual station-level confounding rather than leaving it in the error term.
...
Remaining gaps in `sunshine`, `evaporation`, and cloud cover are addressed through a two-pass Random Forest approach. The first pass uses `missRanger` with Predictive Mean Matching across the full dataset, preserving marginal distributions and preventing variance shrinkage. The second pass applies targeted `mice` with Random Forest, imputing each variable from a scientifically motivated predictor set: cloud cover from humidity and pressure, sunshine from cloud cover and temperature, and evaporation from wind speed, temperature, humidity, sunshine, and location.

The use of targeted predictor sets is directly motivated by the co-missingness finding. Because the four variables tend to be absent together at the station level, their cross-variable correlations when observed are strong and reliable signals for imputation. Restricting each variable's predictor set to physically motivated covariates exploits this structure while reducing the risk of imputing on spurious correlations.

### Ghost Sensor Sanitisation

After the pipeline completes, all station-variable pairs identified in @sec-missingness-diagnostics as ghost sensors are reverted to missing. The justification follows from the ghost sensor analysis: a Random Forest model has no empirical support for predicting values at a station where an instrument was never present. Synthetic values produced for these pairs would be statistically credible by construction but carry no physical information. Accepting the remaining missingness is the honest response.
...
The three-stage pipeline produces a dataset whose properties are each traceable to a specific finding from the diagnostic analysis.
...
**Distributional fidelity.** Predictive Mean Matching in Stage 3 ensures that imputed values are drawn from the observed empirical distribution of each variable. The co-missingness analysis justifies heavy use of cross-variable predictors: because these variables fail together at the station level, observed values from correlated variables at the same station constitute the strongest available imputation signal.

**Outcome independence.** The weather-conditionality test confirmed that sunshine missingness does not covary with rainfall amounts. Imputed sunshine values therefore do not introduce a directional bias into the training labels, and the missingness flags serve as optional controls rather than necessary corrections.
