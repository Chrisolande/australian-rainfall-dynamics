...
zi_m2 <- ~ humidity3pm + dewpoint_9am + rain_yesterday +
  cloud_development + pressure_change

zi_m3_to_m5 <- ~ humidity3pm + dewpoint_9am + rain_yesterday +
  cloud_development + pressure_change + sunshine + evaporation

zi_m6 <- ~ humidity3pm + dewpoint_9arun_fitsm + rain_yesterday +
  cloud_development + 
  pressure_change + 
  sunshine + evaporation + 
  diag(1 | location) + 
  humidity_ma7 + day_cos + day_sin

# conditional formulas
m1_cond <- rainfall ~ 1 + humidity3pm + dewpoint_9am +
  dewpoint_change + pressure_change

m2_cond <- rainfall ~ 1 + humidity3pm + dewpoint_9am +
  dewpoint_change + pressure_change +
  day_cos + day_sin

m3_cond <- rainfall ~ 1 + humidity3pm + dewpoint_9am +
  dewpoint_change + pressure_change +
  day_cos + day_sin +
  rainfall_ma7 + days_since_rain + humidity_ma7 + rain_yesterday

m4_cond <- rainfall ~ 1 + humidity3pm + dewpoint_9am +
  dewpoint_change + pressure_change +
  day_cos + day_sin +
  rainfall_ma7 + days_since_rain + humidity_ma7 + rain_yesterday +
...
m5_cond <- rainfall ~ 1 + humidity3pm + dewpoint_9am +
  dewpoint_change + pressure_change +
  day_cos + day_sin +
  rainfall_ma7 + days_since_rain + humidity_ma7 + rain_yesterday +
  sunshine + evaporation + instability_index +
...
m6_cond <- rainfall ~
  humidity3pm + dewpoint_9am + day_sin +
  rainfall_ma7 + humidity_ma7 +
  ns(dewpoint_change) +
  ns(pressure_change) +
  rain_yesterday + sunshine + ns(evaporation) +
...
disp_formula = ~ humidity3pm + rainfall_ma7 + day_sin + day_cos +
                pressure_change + dewpoint_change +
                ns(evaporation) + ns(gust_U_EW, df = 2)
...
render_pooled_model_table(get_model("m1_moisture"), "Model 1: Moisture and Pressure Dynamics")
```
...
compare_models(
  m_large = get_model("m1_moisture"),
  m_small = get_model("m0_null")
) %>% 
...
#| label: clean-up-1
#| include: false
evict_model("m0_null", "m1_moisture")
```
...
render_pooled_model_table(get_model("m2_temporal"), "Model 2: Seasonality and Persistence Effects")
```
...
compare_models(
  m_large = get_model("m2_temporal"),
  m_small = get_model("m1_moisture")
) %>% 
  render_comparison_table(names = c("+ Seasonality & Persistence", "Moisture Only"))

evict_model("m1_moisture", "m2_temporal")
```
```{r}
#| label: clean-up-2
#| include: false
...
render_pooled_model_table(get_model("m3_history"), "Model 3: Accumulated Weather History")
```
...
compare_models(
  m_large = get_model("m3_history"),
  m_small = get_model("m2_temporal")
) %>% 
  render_comparison_table(names = c("+ Accumulated History", "Seasonality & Persistence"))
```

```{r}
#| label: clean-up-3
#| include: false
evict_model("m2_temporal", "m3_history")
```
...
render_pooled_model_table(get_model("m4_energy"), "Model 4: Thermodynamic Energy and Interactions")
```
...
compare_models(
  m_large = get_model("m4_energy"),
  m_small = get_model("m3_history")
) %>% 
  render_comparison_table(names = c("+ Thermodynamics & Interaction", "Accumulated History"))
...
#| label: clean-up-4
#| include: false
evict_model("m3_history", "m4_energy")
```
...
compare_models(
  m_large = get_model("m5_wind"),
  m_small = get_model("m4_energy")
) %>% 
  render_comparison_table(names = c("+ Wind Vectors", "Thermodynamics & Interaction"))
...
compare_models(
  m_large = get_model("m6_mixed"),
  m_small = get_model("m5_wind")
) %>% 
