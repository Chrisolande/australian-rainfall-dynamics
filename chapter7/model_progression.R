...
# run_d1_comparisons() - runs all six nested comparisons sequentially,
...
run_d1_comparisons <- function() {
  pairs <- list(
    list(
      large = "m1_moisture",
      small = "m0_null",
      names = c("+ Moisture & Pressure", "Null Baseline")
    ),
    list(
      large = "m2_temporal",
      small = "m1_moisture",
    ...
    list(
      large = "m6_mixed",
      small = "m5_wind",
      names = c("+ Random Effects", "Wind Vectors")
    )
  )

  for (p in pairs) {
    m_large <- get_model(p$large)
    m_small <- get_model(p$small)

    compare_models(m_large = m_large, m_small = m_small) %>%
      render_comparison_table(names = p$names)
